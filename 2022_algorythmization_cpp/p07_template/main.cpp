...
public:
    // constructor
    explicit CIndex(const T_ & array, const C_ & cmp = std::less<typename T_::value_type>()) : array(array), comparator(cmp) {
    ...
    set<size_t> search (const T_ & sub) const {
        ...
        for (size_t i = 0; i <= N - M; i++, arrIt++) {
            size_t j;

               /* For current index i, check for pattern match */
            auto tempArrIt = arrIt; auto subIt = sub.begin();
            for (j = 0; j < M; j++, tempArrIt++, subIt++)
                if (comparator(*tempArrIt, *subIt) || comparator(*subIt, *tempArrIt))
                    break;

            if (j == M) // if pat[0...M-1] = txt[i, i+1, ...i+M-1]
                result.insert(i);
        }
        if (M == 0 && !result.empty()) result.erase(--result.end());
        return result;
    }
private:
    T_ array;
...
// nekde pouzivam T_ mesto ::value_type
// chybi jakysyk const
