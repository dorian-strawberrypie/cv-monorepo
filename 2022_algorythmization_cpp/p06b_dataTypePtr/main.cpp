...
class CLowerThanComparator {
public:
    bool operator () (const T_ & a, const T_ & b) const {
        return a < b;
    }
};
template <typename T_, typename C_ = CLowerThanComparator<T_>>
class CIndex
{
public:
    // constructor
    CIndex(T_ array, C_ cmp = CLowerThanComparator<T_>()) : array(array), comparator(cmp) {

    }
    set<size_t> search (T_ sub) {
