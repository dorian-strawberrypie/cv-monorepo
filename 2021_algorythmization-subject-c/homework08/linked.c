...
TEMPLOYEE        * newEmployee  ( const char      * name,
                                  TEMPLOYEE       * next )
{
  TEMPLOYEE * t = (TEMPLOYEE*)malloc(sizeof(TEMPLOYEE));
...
{
  TEMPLOYEE * iter_src = src;
  // TEMPLOYEE * iter_clone = clone;

  TEMPLOYEE * clone_iter = src ? (TEMPLOYEE*)malloc(sizeof(TEMPLOYEE)) : NULL, * clone = clone_iter;
  while (iter_src) {
    // instance allocation
    clone_iter = clone_iter ? clone_iter : (TEMPLOYEE*)malloc(sizeof(TEMPLOYEE));

    // instance initialization
    clone_iter->m_Name = strdup(iter_src->m_Name);
    clone_iter->m_Bak = NULL;
    clone_iter->m_Next = iter_src->m_Next ? (TEMPLOYEE *)malloc(sizeof(TEMPLOYEE)) : NULL;
    
    // instance = isntance.next
    clone_iter = clone_iter->m_Next;
    iter_src = iter_src->m_Next;
  }
  // TODO: find all m_Bak
  return clone;
}
...
#ifndef __PROGTEST__
void iterateList(TEMPLOYEE * src) {
  TEMPLOYEE * iter = src;
  while (iter) {
    printf("n'%s' b'%d' i'%d''\n", iter->m_Name, iter->m_Bak, iter);
    iter = iter->m_Next;
  }
}
int                main         ( int               argc, 
